...
|---|---|---|---|---|
| DATA-1 | Run broker-enabled recommendations during US regular hours (21:30–04:00 SGT) with directional questions, on several tickers, for several weeks | Every stored run so far is NEUTRAL or brokerless, so there are no scored directional outcomes; calibration needs 30 of them at 20 trading days (outcomes.calibration.min-samples), the track record has no hit rates, and the critic's effect cannot be measured | TWS logged in and reachable; a recurring request schedule (a cron or a scheduled task hitting POST /api/recommendations) | OPEN |
...
| AGENT-4 | Direction policy: risk questions reliably produce NEUTRAL | If most runs stay NEUTRAL, DATA-1, DATA-3, DATA-4, DATA-5, and AGENT-2 starve regardless of broker uptime | Decide whether the manager should always state a direction when levels exist, or whether request templates for scheduled runs should ask directional questions | DECISION |
