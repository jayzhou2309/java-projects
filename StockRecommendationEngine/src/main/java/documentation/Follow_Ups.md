...
| AGENT-9 | Provider token-per-minute allowance: the OpenAI organisation is limited to 30,000 TPM for gpt-4.1, and one run with a critic revision uses up to about 30,000 tokens | Back-to-back runs (the watchlist, or a manual request right after a scheduled one) hit HTTP 429; found live on 2026-09-12 when the second watchlist ticker's critic call was rejected. The single retry and the 60-second watchlist pause work around it; they do not remove the ceiling, and a third concurrent caller would still fail | Request a rate-limit increase from OpenAI, or route the critic to a second model/provider (AGENT-1), or trim the critic's context (cited passages only, no full track record) | OPEN |
